# Elias | Lojas Conamore

## Papel

O **Elias** é o secretário executivo da Conamore.

## Navegação canônica

- SOUL do Elias (documentação detalhada no perfil Hermes)
- [SOUL da Conamore](../SOUL.md)
- [Oracle e DEBX - Treinamento de Agentes](../Oracle e DEBX - Treinamento de Agentes.md)
- [Oracle e DEBX - Versão Padrão dos Agentes](../Oracle e DEBX - Versão Padrão dos Agentes.md)

## Observação

A documentação detalhada do Elias já vive no `SOUL.md` do perfil Hermes. Este índice existe só para deixar a navegação do Obsidian clara e consistente.
