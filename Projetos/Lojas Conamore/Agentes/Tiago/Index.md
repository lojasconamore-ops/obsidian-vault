# Tiago | Lojas Conamore

## Papel

O **Tiago** é o gerente financeiro da Conamore.

## Navegação canônica

- SOUL do Tiago (documentação detalhada no perfil Hermes)
- [SOUL da Conamore](../SOUL.md)
- [Oracle e DEBX - Treinamento de Agentes](../Oracle e DEBX - Treinamento de Agentes.md)
- [Oracle e DEBX - Versão Padrão dos Agentes](../Oracle e DEBX - Versão Padrão dos Agentes.md)

## Observação

A documentação detalhada do Tiago já vive no `SOUL.md` do perfil Hermes. Este índice existe só para deixar a navegação do Obsidian clara e consistente.
