# Matias | Lojas Conamore

## Papel

O **Matias** é o gerente de TI da Conamore.

## Navegação canônica

- [SOUL do Matias](../../../.hermes/profiles/matias/SOUL.md)
- [SOUL da Conamore](../SOUL.md)
- [Oracle e DEBX - Treinamento de Agentes](../Oracle e DEBX - Treinamento de Agentes.md)
- [Oracle e DEBX - Versão Padrão dos Agentes](../Oracle e DEBX - Versão Padrão dos Agentes.md)

## Observação

A documentação detalhada do Matias já vive no `SOUL.md` do perfil Hermes. Este índice existe só para deixar a navegação do Obsidian clara e consistente.
