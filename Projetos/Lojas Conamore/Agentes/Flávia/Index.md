# Flávia | Lojas Conamore

## Papel

A **Flávia** é a gerente de marketing da Conamore.

## Navegação canônica

- [SOUL da Flávia](../../../.hermes/profiles/marketing/SOUL.md)
- [SOUL da Conamore](../SOUL.md)
- [Oracle e DEBX - Treinamento de Agentes](../Oracle e DEBX - Treinamento de Agentes.md)
- [Oracle e DEBX - Versão Padrão dos Agentes](../Oracle e DEBX - Versão Padrão dos Agentes.md)

## Observação

A documentação detalhada da Flávia já vive no `SOUL.md` do perfil Hermes `marketing`. Este índice existe só para deixar a navegação do Obsidian clara e consistente.
